...
vocab = mcgan_data_helpers.build_vocab()
# embeddings =mcgan_data_helpers.load_vectors(vocab)
alist = mcgan_data_helpers.read_alist()
raw = mcgan_data_helpers.read_raw()
...
def dev_step(sess,cnn,testList,dev_size=100):
    ...
    for i in range(dev_size):
        ...
        for j in range(int(500/FLAGS.batch_size)):
            x_test_1, x_test_2, x_test_3 = mcgan_data_helpers.load_val_batch(testList, vocab, i*500+j*FLAGS.batch_size, FLAGS.batch_size)
            feed_dict = {
    ...
    with tf.Graph().as_default():
        with tf.device("/gpu:0"):
            ...
            with sess.as_default() ,open(precision,"w") as log:
                    ...
                    # print( param)
                    param= None
                    DIS_MODEL_FILE="train_mcgan/pre-trained.model"
                    ...
                    for i in range(FLAGS.num_epochs):
                        ...
                        for j in range(1):
                            for batch in mcgan_data_helpers.batch_iter(samples,batch_size=FLAGS.batch_size,num_epochs=1,shuffle=True):    # try:                        
