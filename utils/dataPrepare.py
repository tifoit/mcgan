...
answers=load("train_mcgan/answers")
print ("have %d answers" % len(answers))
vocabulary=load("train_mcgan/vocabulary")
print ("have %d words" % len(vocabulary))



def convert2TSV(rawFilename):
    test1=load("train_mcgan/"+rawFilename)
    lines=[]
    print (len(test1))
    for item in test1:
        ...
        for sen in bad:
            ...
            lines.append(line)
        # print lines
    filename= "train_mcgan/mcgan_%s.tsv" %(rawFilename)
...
def parseTrain():
    ...
    filename= "train_mcgan/mcgan_%s.tsv" %("train")
    with open(filename, "w") as f:
        f.write("\n".join(lines) )

def format_file(filename="mcgan_dev.tsv",subset_size=1800):
    temp_file="mcganQA"+"/"+filename[filename.index("_")+1:filename.index(".")]
