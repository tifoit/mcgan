...
def build_vocab():
    ...
    vocab['UNKNOWN'] = code
    code += 1
    for filename in ['train_mcgan/train','train_mcgan/test1','train_mcgan/test2','train_mcgan/dev']:
...
def read_alist():
    alist = []
    for line in open('train_mcgan/train'):
        items = line.strip().split(' ')
...
def load_vectors(vocab=None):
    if vocab==None:
        return
    vectors = {}
    for line in open('train_mcgan/vectors.nobin'):
        items = line.strip().split(' ')
        if (len(items) < 101):
            continue
        vec = []
...
def loadTestSet(filename):
    testList = []
    for line in open('train_mcgan/'+filename):
