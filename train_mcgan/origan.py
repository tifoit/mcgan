...
print(("Loading data..."))
vocab = mcgan_data_helpers.build_vocab()  # '/train',/test1',/test2',/dev'
# with open("mcgan/vocab","w") as out_op:
#     for v in vocab:
#         result=v+"\t"+str(vocab[v])+"\n"
#         out_op.write(result)

# embeddings =mcgan_data_helpers.load_vectors(vocab)
alist = mcgan_data_helpers.read_alist()  # [...]  ('mcgan/train')  A 全部的answer数据
raw = mcgan_data_helpers.read_raw()  # [...]  ('mcgan/train')     Q,A 正的数据

test1List = mcgan_data_helpers.loadTestSet("test1")
test2List = mcgan_data_helpers.loadTestSet("test2")
devList = mcgan_data_helpers.loadTestSet("dev")
...
def generate_gan(sess, model, loss_type="pair", negative_size=3):
    ...
    for _index, pair in enumerate(raw):
        ...
        canditates = mcgan_data_helpers.loadCandidateSamples(q, a, pools, vocab)
        predicteds = []
        for batch in mcgan_data_helpers.batch_iter(canditates, batch_size=FLAGS.batch_size):
...
def dev_step(sess, cnn, testList, dev_size=100):
    ...
    for i in range(dev_size):
        ...
        for j in range(int(500 / FLAGS.batch_size)):
            x_test_1, x_test_2, x_test_3 = mcgan_data_helpers.load_val_batch(testList, vocab,
                ...
                for i in range(FLAGS.num_epochs):
                    if i > 0:
                        ...
                        for _index, batch in enumerate(
                                mcgan_data_helpers.batch_iter(samples, num_epochs=FLAGS.d_epochs_num,
                                                                     batch_size=FLAGS.batch_size,
                                                                     shuffle=True)):  # try:
                    ...
                    for g_epoch in range(FLAGS.g_epochs_num):  ## 1
                        ...
                        loss_log.write("MC pairs have " + str(len(raw)) + "\n")
                        for _index, pair in enumerate(raw):
                            q = pair[2]
                            a = pair[3]
                            ...
                            pools = np.array(alist)[sampled_index]  # 
                            samples = mcgan_data_helpers.loadCandidateSamples(q, a, pools, vocab)  

                            ## 1
                            predicteds = []
                            for batch in mcgan_data_helpers.batch_iter(samples, batch_size=FLAGS.batch_size):
                            ...
                            subsamples = np.array(
                                mcgan_data_helpers.loadCandidateSamples(q, a, pools[neg_index], vocab))
