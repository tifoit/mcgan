...
# define model
predict_params = train_mcgan_utils.init_predict_params(nz=nz, n_f=n_f, n_layers=n_layers)
print('load predictive model from %s' % model_dir)
train_mcgan_utils.load_model(predict_params, os.path.join(model_dir, 'predict_params'))

x = T.tensor4()
z_p, bn_data = train_mcgan_utils.predict_batchnorm(x, predict_params, n_layers=n_layers)

print('COMPILING...')
t = time()
_estimate_batchnorm = function([x], bn_data)
