...
import train_mcgan_config
from lib import updates
from lib import utils
from lib.rng import py_rng, np_rng
from lib import costs
import train_mcgan_utils
import load
from lib import image_save
import argparse

# set arguments and parameters
parser = argparse.ArgumentParser('Train mcgan model')
...
n_vis = 196
npx, n_layers, n_f, nc, nz, niter, niter_decay = getattr(train_mcgan_config, args.model_name)()
...
web_dir = os.path.join(args.cache_dir, 'web_mcgan')
html = image_save.Save(web_dir, expr_name, append=True)
...
test_x = train_mcgan_utils.transform(test_x, nc=nc)
vis_idxs = py_rng.sample(np.arange(len(test_x)), n_vis)
vaX_vis = train_mcgan_utils.inverse_transform(test_x[vis_idxs], npx=npx, nc=nc)
# st()
n_grid = int(np.sqrt(n_vis))
grid_real = utils.grid_vis((vaX_vis * 255.0).astype(np.uint8), n_grid, n_grid)
train_mcgan_utils.save_image(grid_real, os.path.join(sample_dir, 'real_samples.png'))


# define mcgan model
disc_params = train_mcgan_utils.init_disc_params(n_f=n_f, n_layers=n_layers, nc=nc)
gen_params = train_mcgan_utils.init_gen_params(nz=nz, n_f=n_f, n_layers=n_layers, nc=nc)
x = T.tensor4()
z = T.matrix()

gx = train_mcgan_utils.gen(z, gen_params, n_layers=n_layers, n_f=n_f, nc=nc)
p_real = train_mcgan_utils.discrim(x, disc_params, n_layers=n_layers)
p_gen = train_mcgan_utils.discrim(gx, disc_params, n_layers=n_layers)
...
print('COMPILING')
t = time()
_train_g = function([x, z], cost, updates=g_updates)
_train_d = function([x, z], cost, updates=d_updates)
_gen = function([z], gx)
print('%.2f seconds to compile functions' % (time() - t))
...
for epoch in range(niter + niter_decay):
    for imb, in tqdm(tr_stream.get_epoch_iterator(), total=ntrain / args.batch_size):
        imb = train_mcgan_utils.transform(imb, nc=nc)
    ...
    samples_t = train_mcgan_utils.inverse_transform(samples, npx=npx, nc=nc)
    grid_vis = utils.grid_vis(samples_t, n_grid, n_grid)
    grid_vis_i = (grid_vis * 255.0).astype(np.uint8)
    train_mcgan_utils.save_image(grid_vis_i, os.path.join(sample_dir, 'gen_%5.5d.png' % n_epochs))
    ...
    if n_epochs % args.save_freq == 0:
        train_mcgan_utils.save_model(disc_params, '%s/disc_params_%3.3d' % (model_dir, n_epochs))
        train_mcgan_utils.save_model(gen_params, '%s/gen_params_%3.3d' % (model_dir, n_epochs))
    train_mcgan_utils.save_model(disc_params, '%s/disc_params' % model_dir)
    train_mcgan_utils.save_model(gen_params, '%s/gen_params' % model_dir)
