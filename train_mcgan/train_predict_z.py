...
predict_params = train_mcgan_utils.init_predict_params(nz=nz, n_f=n_f, n_layers=n_layers, nc=nc)
# load modelG
gen_params = train_mcgan_utils.init_gen_params(nz=nz, n_f=n_f, n_layers=n_layers, nc=nc)
train_mcgan_utils.load_model(gen_params, os.path.join(model_dir, 'gen_params'))
gen_batchnorm = train_mcgan_utils.load_batchnorm(os.path.join(model_dir, 'gen_batchnorm'))

# define the model
t = time()
x = T.tensor4()
z = train_mcgan_utils.predict(x, predict_params, n_layers=n_layers)
gx = train_mcgan_utils.gen_test(z, gen_params, gen_batchnorm, n_layers=n_layers, n_f=n_f)
...
print('COMPILING')
t = time()
_train_p = function([x], cost, updates=p_updates)
_train_p_cost = function([x], [cost, gx])
_predict_z = function([x], z)
_gen = function([z], gx)
...
def rec_test(test_data, n_epochs=0, batch_size=128, output_dir=None):
    ...
    print('computing reconstruction loss on test datas')
    rec_imgs = []
    imgs = []
    costs = []
    ntest = len(test_data)
    ...
    for n in tqdm(range(ntest / batch_size)):
        ...
        # imb = train_mcgan_utils.transform(xmb, nc=3)
        ...
        if n == 0:
            ...
            imgs.append(train_mcgan_utils.inverse_transform(imb, npx=npx, nc=nc))
            rec_imgs.append(train_mcgan_utils.inverse_transform(gx, npx=npx, nc=nc))
...
for epoch in range(niter + niter_decay):
    ...
    for xmb, in tr_stream.get_epoch_iterator():
        imb = train_mcgan_utils.transform(xmb, nc=nc)
        n += 1
    ...
    if n_epochs % args.save_freq == 0:
        train_mcgan_utils.save_model(predict_params, '%s/predict_params%3.3d' % (model_dir, n_epochs))
    train_mcgan_utils.save_model(predict_params, os.path.join(model_dir, 'predict_params'))  # save the latest one
