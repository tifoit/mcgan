...
parser = argparse.ArgumentParser('compute batchnorm statistics for mcgan model')
...
train_mcgan_utils.load_model(disc_params, os.path.join(model_dir, 'disc_params'))
train_mcgan_utils.load_model(gen_params, os.path.join(model_dir, 'gen_params'))

Z = T.matrix()
gX, gbn = train_mcgan_utils.gen_batchnorm(Z, gen_params, n_layers=n_layers, n_f=n_f, nc=nc)
p_gen, dbn = train_mcgan_utils.discrim_batchnorm(gX, disc_params, n_layers=n_layers)
ngbn = len(gbn)
ndbn = len(dbn)
bn_data = gbn + dbn
...
print('COMPILING...')
t = time()
_estimate_bn = function([Z], bn_data)
