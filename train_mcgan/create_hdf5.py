...
if os.path.isfile(args.hdf5_file):  # if hdf5 file already exists
    ...
    if args.mode == 'dir':  # if datas are stored in a directory
        imgList = os.listdir(args.dataset_dir)
        nImgs = len(imgList)
        np.random.shuffle(imgList)
        print('read %d datas from %s' % (nImgs, args.dataset_dir))
    ...
    print('writing %d datas to hdf5 file %s' % (nImgs, args.hdf5_file))
